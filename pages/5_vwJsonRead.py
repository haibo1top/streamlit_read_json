...
# 关键属性
KEY_PROPERTIES_PATH: str = r'mapfile/vw_key_properties_list.csv'
...
def read_json(jon: str, data_map: tuple):
    ...
    event_info = [str(json_text), json_data['event'], event_name["事件显示名"].iloc[0]]
    # 拼接内容：json报文,事件名,事件中文名,关键属性名,关键属性值
...
def show_json_data():
    ...
    if json_text != "" and is_json(json_text):
        ...
        if event_name["事件名"].iloc[0] in ["$pageview", "$WebClick", "$AppViewScreen", "$AppClick"]:
            st.info("梳理范围内埋点")
        else:
            st.error("不在梳理范围内")
        ...
        r2com1.dataframe(propertie_all.T, width=800, height=600)
        r2com2.markdown("###### 公共属性")
        r2com2.dataframe(propertie.T, width=800, height=600)
        # st.markdown("###### 方案外属性")
